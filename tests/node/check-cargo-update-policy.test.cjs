...
const { runPolicyCheck } = require("../../scripts/check-cargo-update-policy.cjs");
...
function createTempRepo() {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: "test-pkg",
      scripts: {
        u: "node scripts/npm-safe-update.cjs && node scripts/cargo-safe-update.cjs && node scripts/sync-version.cjs",
      },
    }),
  );
  mkdirSync(path.join(tempDir, "scripts"), { recursive: true });
  writeFileSync(
    path.join(tempDir, "scripts", "cargo-safe-update.cjs"),
    "// approved implementation\ncargo update\n",
  );
  writeFileSync(
    path.join(tempDir, "scripts", "npm-safe-update.cjs"),
    "// approved implementation\nnpm update\n",
  );
  return tempDir;
}
...
test("3. blocks guarded plus raw update in one chained command", () => {
  ...
  try {
    writeFileSync(
      ...
      JSON.stringify({
        scripts: {
          u: "node scripts/cargo-safe-update.cjs && cargo update",
        },
      }),
    );
    const errors = [];
...
test("12. requires exact path exclusion (tools/cargo-safe-update.cjs is blocked)", () => {
  const root = createTempRepo();
  try {
    mkdirSync(path.join(root, "tools"), { recursive: true });
    writeFileSync(
      path.join(root, "tools", "cargo-safe-update.cjs"),
      "cargo update\n",
    );
    const errors = [];
    const ok = runPolicyCheck({
    ...
    assert.equal(ok, false);
    assert.ok(errors.some((e) => e.includes("tools/cargo-safe-update.cjs")));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("13. blocks cargo-safe-update.cjs && cargo update in a shell file", () => {
  const root = createTempRepo();
  try {
    writeFileSync(
      path.join(root, "scripts", "update.sh"),
      "#!/bin/sh\nnode scripts/cargo-safe-update.cjs --manifest-path src-tauri/Cargo.toml && cargo update\n",
    );
    const errors = [];
    const ok = runPolicyCheck({
      root,
...
test("15a. blocks guarded wrapper + raw update in .ps1 file", () => {
  ...
  try {
    writeFileSync(
      ...
      "node scripts/cargo-safe-update.cjs; cargo update\n",
    );
    const errors = [];
    const ok = runPolicyCheck({
      root,
...
test("15b. blocks guarded wrapper + raw update in .mjs file", () => {
  ...
  try {
    writeFileSync(
      ...
      '// updater\nconst x = "node scripts/cargo-safe-update.cjs";\ncargo update\n',
    );
    const errors = [];
    const ok = runPolicyCheck({
      root,
...
test("20. approved exact helper file (scripts/cargo-safe-update.cjs) remains excluded for programmatic calls", () => {
  // The approved helper internally uses spawnSync("cargo", ...); this must remain excluded
  const root = createTempRepo();
  try {
    // cargo-safe-update.cjs already exists in scripts/ (created by createTempRepo),
    // but its content has "cargo update" not a spawnSync form; override to test:
    writeFileSync(
      path.join(root, "scripts", "cargo-safe-update.cjs"),
