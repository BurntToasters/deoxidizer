...
const { CARGO_SAFE_UPDATE_POLICY_VERSION, CARGO_SAFE_UPDATE_VERSION, MIN_PUBLISH_AGE_MS, acquireUpdateLock, crateIndexPath, filterRegistryIndex, findWorkspaceRoot, locateWorkspaceRoot, installValidatedLock, isPublishAgeAllowed, parseArguments, parsePublishTime, prepareCandidate, restoreRealLock, validateCandidate } = require("../../scripts/cargo-safe-update.cjs");
const { MINIMUM_NPM_VERSION, parseVersion, isVersionAtLeast, isSupportedNodeVersion, pinnedRustChannel, hasPinnedRustToolchain, npmUpdateArguments, npmAuditPlan, npmInvocation, npmUpdateInvocation, restoreSnapshot, usesWindowsCmdShell } = require("../../scripts/npm-safe-update.cjs");
...
test("22. candidate stage implementation never invokes cargo build/check/test/run/bench", () => {
  const source = readFileSync(
    path.join(__dirname, "..", "..", "scripts", "cargo-safe-update.cjs"),
    "utf8",
  );
  assert.doesNotMatch(
    source,
...
test("25. dependency update entry points use guarded Cargo resolution", () => {
  ...
  for (const name of ["u", "update:safe"]) {
    const command = packageJson.scripts?.[name];
    assert.ok(command, `package.json script "${name}" must exist`);
    assert.doesNotMatch(command, /\bcargo update\b/);
    assert.match(command, /cargo-safe-update/);
  }
  const uCommand = packageJson.scripts["u"];
  assert.match(uCommand, /npm-safe-update/);
  assert.match(uCommand, /sync-version\.cjs/);
  // `u` must never execute dependency code (install/build/test/release).
  assert.doesNotMatch(uCommand, /quality|test|build|release/);
});
...
test("30. version markers are exported", () => {
  assert.equal(CARGO_SAFE_UPDATE_POLICY_VERSION, 1);
  assert.equal(CARGO_SAFE_UPDATE_VERSION, 1);
});
...
test("45. update environment versions fail closed at exact boundaries", () => {
  assert.deepEqual(parseVersion("12.0.2"), [12, 0, 2]);
  assert.equal(isVersionAtLeast("12.0.2", MINIMUM_NPM_VERSION), true);
  assert.equal(isVersionAtLeast("12.1.0", MINIMUM_NPM_VERSION), true);
  assert.equal(isVersionAtLeast("12.0.1", MINIMUM_NPM_VERSION), false);
  ...
  assert.equal(isSupportedNodeVersion("26.99.99"), true);
  ...
  assert.equal(
    hasPinnedRustToolchain("1.98.1-aarch64-apple-darwin (default)\n", "1.98.1"),
    true,
  );
  assert.equal(hasPinnedRustToolchain("stable-x86_64-unknown-linux-gnu\n", "1.98.1"), false);
  assert.equal(hasPinnedRustToolchain("1.97.1-aarch64-apple-darwin\n", "1.98.1"), false);
  const toolchainChannel = readFileSync(
    path.join(__dirname, "..", "..", "rust-toolchain.toml"),
    "utf8",
  ).match(/^channel\s*=\s*"([^"]+)"/m)?.[1];
  assert.equal(pinnedRustChannel(), toolchainChannel);
});
...
test("46a. npm update audits production strictly", () => {
  const plan = npmAuditPlan("isolated-workspace", "/isolated/npm-cache", {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...
            RUSTUP_TOOLCHAIN: pinnedRustChannel(),
          },
          encoding: "utf8",
          timeout: 120_000,
        },
...
test("50. end-to-end npm update is lock-only and uses a disposable cache", () => {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "..", "..", "scripts", "npm-safe-update.cjs")],
      {
        cwd: workspace,
