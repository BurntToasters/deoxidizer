...
/* Temporary regression scanner. Remove together with cargo-safe-update.cjs
...
const CARGO_UPDATE_POLICY_SCANNER_VERSION = 1;
const CARGO_UPDATE_SCANNER_VERSION = 1;
...
const EXACT_AUTOMATION_FILES = new Set([
  "Makefile",
  "makefile",
  "GNUmakefile",
  "justfile",
...
const EXCLUDED_FILES = new Set([
  "scripts/cargo-safe-update.cjs",
  "scripts/npm-safe-update.cjs",
  "scripts/check-cargo-update-policy.cjs",
  "tests/node/cargo-safe-update.test.cjs",
  "tests/node/check-cargo-update-policy.test.cjs",
]);
...
function walkExecutableFiles(root, currentDir = root, results = []) {
  let entries;
  try {
...
function scanFile(relPath, fullPath, violations) {
...
function scanPackageJson(root, violations) {
...
function scanCargoConfig(root, violations) {
  const configPaths = [
...
function runPolicyCheck({
  ...
  if (violations.length > 0) {
    ...
    error(
      "Dependency-changing workflows must route through scripts/npm-safe-update.cjs and scripts/cargo-safe-update.cjs.",
    );
    return false;
  }
...
module.exports = { CARGO_UPDATE_POLICY_SCANNER_VERSION, CARGO_UPDATE_SCANNER_VERSION, EXECUTABLE_EXTENSIONS, EXACT_AUTOMATION_FILES, IGNORED_DIRECTORIES, EXCLUDED_FILES, normalizeRelPath, commandSegments, collapseLineContinuations, stripCommentLines, classifyLine, walkExecutableFiles, scanFile, scanPackageJson, scanCargoConfig, runPolicyCheck, checkCargoUpdatePolicy: runPolicyCheck };
