...
const { closeSync, copyFileSync, cpSync, existsSync, fsyncSync, lstatSync, mkdtempSync, openSync, readFileSync, renameSync, rmSync, writeFileSync } = require("node:fs");
...
const CARGO_SAFE_UPDATE_POLICY_VERSION = 1;
const CARGO_SAFE_UPDATE_VERSION = 1;
const MIN_PUBLISH_AGE_MS = 72 * 60 * 60 * 1000;
...
function isPublishAgeAllowed(publishTime, now = Date.now()) {
  return (
...
function crateIndexPath(crateName) {
...
function cargoSupportsTemporaryLockfile(env = process.env) {
...
function acquireUpdateLock(workspaceRoot) {
...
function restoreRealLock(realLock, original) {
...
module.exports = { CARGO_SAFE_UPDATE_POLICY_VERSION, CARGO_SAFE_UPDATE_VERSION, MIN_PUBLISH_AGE_MS, parseArguments, parsePublishTime, isPublishAgeAllowed, crateIndexPath, manifestPath, selectedPackages, cargoSupportsTemporaryLockfile, locateWorkspaceRoot, findWorkspaceRoot, prepareCandidate, filterRegistryIndex, validateCandidate, acquireUpdateLock, installValidatedLock, restoreRealLock };
